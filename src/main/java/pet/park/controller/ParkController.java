...
public ContributorData retrieveContributorById(@PathVariable Long contributorId) {
	log.info("Retrieve all contributor with ID= {}",contributorId);
...
public Map< String ,String>deleteContributorById(
	...
	log.info("Deleting contributor with ID= {}",contributorId);
	 ...
	 return Map.of("message", "Delete contributor with ID=" + contributorId +
			 " was successful.");
}
@PostMapping("/contributor/{contribtuorId}/park")
@ResponseStatus(code =HttpStatus.CREATED)
public PetParkData insertPetPark(@PathVariable Long contributorId, 
@RequestBody PetParkData petParkData) {
	
	log.info("Creating park {} for contributor with ID= {}",
			petParkData,contributorId);	
	return parkService.savePetPark(contributorId, petParkData);
}

@PutMapping("/contributor/{contribtuorId}/park{parkId}")
...
public PetParkData retrievePetParkById(@PathVariable Long contributorId, 
		@PathVariable Long parkId) {
	log.info("Retieving pet park with ID= {} for contributor with ID= {} ",
			parkId,contributorId);
	return  parkService.retrievePetParkId(contributorId,parkId);
}
}
